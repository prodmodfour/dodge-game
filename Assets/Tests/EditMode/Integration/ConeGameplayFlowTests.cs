...
public sealed class ConeGameplayFlowTests
{
    [Test]
    public void PlayerCanPreviewDeclareConeTargetReactsOutAndConeIsAvoidedByMovement()
    {
        using (var fixture = new Fixture())
        {
            ...
            fixture.AssignLoadout(actor, fixture.Move, fixture.ConeShot, fixture.PassReaction);
            ...
            var expectedConeCells = AreaShapeService.GetConeCells(
                actor.CurrentGridPosition,
                CardinalDirection.East,
                fixture.ConeShot.Range,
                fixture.GridManager.CurrentMap).ToArray();
            Assert.That(fixture.DangerPreview.CurrentPreview.Ability, Is.SameAs(fixture.ConeShot));
            Assert.That(fixture.DangerPreview.CurrentPreview.Target.TargetCell, Is.EqualTo(aimCell));
            Assert.That(fixture.DangerPreview.CurrentPreview.AffectedCells.ToArray(), Is.EqualTo(expectedConeCells));
            ...
            Assert.That(
                ...
                Is.EquivalentTo(expectedConeCells));
            Assert.That(fixture.DangerPreview.LastFeedback, Does.Contain("Cone Shot"));
            ...
            var targetHpBeforeCone = target.CurrentHP;
            ...
            Assert.That(pendingIntent.Ability, Is.SameAs(fixture.ConeShot));
            ...
            Assert.That(pendingIntent.DeclaredAffectedCells.ToArray(), Is.EqualTo(expectedConeCells));
            Assert.That(actor.CurrentAP, Is.EqualTo(actor.MaxAP - fixture.ConeShot.APCost));
            ...
            var safeDestination = new GridPosition(2, 0, 3);
            ...
            Assert.That(safetyCell.Reason, Does.Contain("outside the declared cone"));
            ...
            Assert.That(target.CurrentHP, Is.EqualTo(targetHpBeforeCone));
            ...
            Assert.That(combatLog.Any(line => line.Contains("Reaction order for Cone Shot")), Is.True);
            Assert.That(combatLog.Any(line => line.Contains("reaction-moved") && line.Contains("Cone Shot")), Is.True);
            Assert.That(combatLog.Any(line => line.Contains("resolved as a cone") && line.Contains("avoided by movement")), Is.True);
    ...
    private sealed class Fixture : IDisposable
    {
        ...
        public Fixture()
        {
            ...
            var registryObject = CreateRoot("Cone Gameplay Flow Registry");
            var gridObject = CreateRoot("Cone Gameplay Flow Grid");
            var inputObject = CreateRoot("Cone Gameplay Flow Input");
            var managerObject = CreateRoot("Cone Gameplay Flow Manager");
            var highlightObject = CreateRoot("Cone Gameplay Flow Highlights");
            var dangerPreviewObject = CreateRoot("Cone Gameplay Flow Danger Preview");
            var safetyPreviewObject = CreateRoot("Cone Gameplay Flow Safety Preview");
            var cameraObject = CreateRoot("Cone Gameplay Flow Camera");
            ...
            bool triggersReactions)
        {
            ...
            ability.Configure(
                ...
                damage,
                triggersReactions,
                description: "Cone gameplay flow test ability.");
            return ability;
        }
        ...
        private void RegisterTiles()
        {
            for (var x = 0; x < 5; x += 1)
            {
                for (var z = 0; z < 4; z += 1)
                {
                    ...
                    tileObject.name = $"Cone Gameplay Flow Tile {x},{z}";
